...
describe('Session Middleware', () => {
  it('should allow access to public routes without session', async () => {
    ...
    const request = new NextRequest(new URL('http://localhost:3026/'))
  ...
  it('should protect auth routes without valid session', async () => {
    ...
    const request = new NextRequest(new URL('http://localhost:3026/dashboard'))
    const response = await sessionMiddleware(request)
  ...
  it('should skip middleware for API routes', async () => {
    ...
    const request = new NextRequest(new URL('http://localhost:3026/api/test'))
    const response = await sessionMiddleware(request)
  ...
  it('should skip middleware for static assets', async () => {
    ...
    const request = new NextRequest(new URL('http://localhost:3026/_next/static/test.js'))
    const response = await sessionMiddleware(request)
