...
program.name('fedsync-import').description('Import FedSync data into Payload CMS').version('1.0.0')
  ...
  .action(async (dataPath: string, options: any) => {
    ...
    try {
      ...
      const importOptions: ImportOptions = {
        ...
        logLevel: logLevel,
        logFile: options?.logFile ?? 'logs/importReport.log',
      }
      ...
      // Success summary
      const totalImported =
        stats.categories.imported + stats.events.imported + stats.profiles.imported
...
program
  ...
  .action(() => {
    ...
    console.log(
      chalk.gray('  fedsync-import import /path/to/data --batch-size 100 --concurrency 10\n'),
    )
    ...
    console.log(
      chalk.gray('  fedsync-import import /path/to/data --log-level debug --log-file import.log\n'),
    )
